...
namespace static_meshes_3D {
	...
	Cone::Cone(float radius, int numSlices, float height, bool withPositions, bool withTextureCoordinates, bool withNormals, bool withRotation)
	...
	void Cone::initializeData()
	{
		...
		if (hasPositions())
		{
			if (_rotation)
			{
				...
				for (auto i = 0; i <= _numSlices; i++)
				{
					const auto topPosition = glm::vec3(0.0f, 0.0f, _height / 2.0f);
			...
			else
			{
				...
				for (auto i = 0; i <= _numSlices; i++)
				{
					const auto topPosition = glm::vec3(0.0f, _height / 2.0f, 0.0f);
