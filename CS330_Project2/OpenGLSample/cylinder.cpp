...
namespace static_meshes_3D {
	...
	Cylinder::Cylinder(float radius, int numSlices, float height, bool withPositions, bool withTextureCoordinates, bool withNormals, bool withRotation, bool cup)
		: StaticMesh3D(withPositions, withTextureCoordinates, withNormals)
		, _radius(radius)
		, _numSlices(numSlices)
		, _height(height)
		, _rotation(withRotation)
		, _cup(cup)
	{
		initializeData();
	}
	...
	void Cylinder::initializeData()
	{
		...
		if (hasPositions())
		{
			if (_rotation)
			{
				...
				for (auto i = 0; i <= _numSlices; i++)
				{
					y.push_back(cosines[i] * _radius);
					x.push_back(sines[i] * _radius);
				}

				// Add cylinder side vertices
				for (auto i = 0; i <= _numSlices; i++)
				{
					const auto topPosition = glm::vec3(x[i], y[i], _height / 2.0f);
					const auto bottomPosition = glm::vec3(x[i], y[i], -_height / 2.0f);
					_vbo.addRawData(&topPosition, sizeof(glm::vec3));
					_vbo.addRawData(&bottomPosition, sizeof(glm::vec3));
				}

				// Add top cylinder cover
				glm::vec3 topCenterPosition(0.0f, 0.0f, _height / 2.0f);
				...
				glm::vec3 bottomCenterPosition(0.0f, 0.0f, -_height / 2.0f);
